...
def make_env(log_dir, cpu):
    ...
    def _init():
        env = ObstacleTowerEnv('./ObstacleTower/obstacletower', worker_id=seed+cpu,
                               retro=True, config={'total-floors': 12, 'visual-theme': 0, 'default-theme': 0},
                               greyscale=True, timeout_wait=600)
...
if __name__ == "__main__":
    ...
    multimodel = multimodel.load('models/ppo/multimodel_fix0', multienv)
    multimodel.learn(total_timesteps=args.num_timesteps)
    multimodel.save('models/ppo/multimodel_fix0')
