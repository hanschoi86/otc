...
class DQNAgent(object):
  ...
  def __init__(self,
    ...
    if fetch_key:
      self.observation_shape = tuple((7064,))
    else:
    ...
    with tf.device(tf_device):
      ### enable Random Network Distillation (RND)
      self.enable_rnd = enable_rnd

      ### settings for RND
      if self.enable_rnd:
        self._rnd_batch_size = rnd_batch_size
        self._rnd_normalize_rewards = rnd_normalize_rewards
        self._rnd_initialize = True
      ...
      ### set batch size for RND
      if self.enable_rnd:
        self._rnd_batch = tf.random.uniform(self._replay.rewards.shape)     
      ...
      # set RND update op
      if self.enable_rnd:
        self._rnd_update_op = self._build_rnd_update_op()
  ...
  ### target network for RND
  def _rnd_target_network(self, inputs):
    ...
    output = slim.fully_connected(net, 54, activation_fn=None, trainable=False)
    return output

  ### predictor network for RND
  def _rnd_predict_network(self, inputs):
    net = tf.cast(inputs, tf.float32)
    net = tf.div(net, 255.)
    ...
    output = slim.fully_connected(net, 54, activation_fn=None, trainable=True)
    return output
  ...
  def _build_target_q_op(self):
    ...
    if self.enable_rnd:
      ### pass next state to predictor and target network
      next_obs =  tf.stop_gradient(self._replay.next_states)
      target_output = self._rnd_target_network(next_obs)
      predict_output = self._rnd_predict_network(next_obs)

      ### calculate intrinsic reward as the loss
      loss = tf.squared_difference(target_output, predict_output)
      self.intrinsic_rewards = tf.reduce_sum(loss, axis=1)
      self.rnd_total_loss = tf.reduce_sum(self.intrinsic_rewards)
      
      ### Normalize intrinsic reward by running std
      if self._rnd_normalize_rewards:
        self.intrinsic_std = tf.nn.moments(self._rnd_batch, 0)[1]
        self.normalized_rewards = tf.divide(self.intrinsic_rewards, self.intrinsic_std)

        return (self._replay.rewards + self.normalized_rewards) + self.cumulative_gamma * replay_next_qt_max * (
          1. - tf.cast(self._replay.terminals, tf.float32))
      else:
        return (self._replay.rewards + self.intrinsic_rewards) + self.cumulative_gamma * replay_next_qt_max * (
          1. - tf.cast(self._replay.terminals, tf.float32))
  ...
  def _build_rnd_update_op(self):
    return tf.train.AdamOptimizer(learning_rate=self._rnd_learning_rate).minimize(self.rnd_total_loss)
  ...
  def _train_step(self):
    ...
    if self._replay.memory.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        ...
        # Maintain RND batch for running mean and std.
        if self.enable_rnd:
          if self._rnd_batch.shape[0] < self._rnd_batch_size:
            self._rnd_batch = tf.concat([self._rnd_batch, self.intrinsic_rewards], axis=0)
          else:
            self._rnd_batch = tf.concat([self._rnd_batch, self.intrinsic_rewards], axis=0)[-self._rnd_batch_size:]
      ...
      if self.enable_rnd:
        self._sess.run(self._rnd_update_op)
