...
def nature_cnn_key(scaled_images, **kwargs):
    """
    CNN from Nature paper.
    ...
    scaled = tf.slice(scaled_images, [0, 0], [-1, 84*84])
    scaled = tf.reshape(scaled, shape=(-1, 84, 84, 1))
    keys = tf.slice(scaled_images, [0, 84*84], [-1, -1])
    activ = tf.nn.relu
    layer_1 = activ(conv(scaled, 'c1', n_filters=32, filter_size=8, stride=4, init_scale=np.sqrt(2), **kwargs))
    layer_2 = activ(conv(layer_1, 'c2', n_filters=64, filter_size=4, stride=2, init_scale=np.sqrt(2), **kwargs))
    layer_3 = activ(conv(layer_2, 'c3', n_filters=64, filter_size=3, stride=1, init_scale=np.sqrt(2), **kwargs))
    layer_3 = conv_to_fc(layer_3)
    layer_3 = tf.concat([layer_3, keys], axis=1)
