...
from model import CnnActorCriticNetworkDefault, RNDModel
from envs import *
from utils import RunningMeanStd, RewardForwardFilter
from arguments_default import get_args
...
def main():
    ...
    model = CnnActorCriticNetworkDefault(input_size, output_size, args.use_noisy_net)
    ...
    while True:
        ...
        if global_step % (args.num_worker * args.num_step * args.save_interval) == 0:
            ...
            checkpoint_list = np.array([int(re.search(r"\d+(\.\d+)?", x)[0]) for x in glob.glob(os.path.join('default_models', args.env_name+'*.model'))])
